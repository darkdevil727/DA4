...
class FileChooserController(RelativeLayout):
    ...
    def _update_files(self, *args, **kwargs):
        ...
        #global file_directory = str(self.path) #self.path = folder directory!!!
...
if __name__ == '__main__':
    ...
    root = Builder.load_file('filechooser.kv')
    ...
    class FileChooserApp(App):
        ...
        def run_script_test(self):
            ...
            return print('PATH: '+gb_path_name)
            
        def run_script(self):
            
            ##################### DATA ANALYSIS SCRIPT STARTS ############################
              
            def find_csv_filenames(path_to_dir, suffix=".csv" ):
                filenames = listdir(path_to_dir)
                return [ filename for filename in filenames if filename.endswith( suffix ) or filename.endswith (suffix.upper()) ]
            
            filenames = find_csv_filenames(gb_path_name+".")
             
            
            def process_data(name):
                
                ########################### FFT and PLOTTING STARTS #########################
                        ...
                        if 'BR' in name:
                            ...
                            df = pd.read_csv(gb_path_name+'\\'+name, sep=',', usecols=col_list, encoding = 'utf8')
                            ...
                            plotter.savefig(gb_path_name+'\\'+new_name+'_processed_'+now+'.jpg')
                            plotter.show()

       
                
                
                ########################### FFT and PLOTTING ENDS ###############
            
            
            for x in range(0,len(filenames)):
                process_data(filenames[x])
            ...
            textpopup(self,title='Summary', text= 'The following files have been processed: \n\n' + filenames_list)
            
            ##################### DATA ANALYSIS SCRIPT ENDS #####################################
            
        
        
        def parse_fp(path_name):
            
            global gb_path_name
            gb_path_name = str(path_name)
            #print(gb_path_name)
            #FileChooserApp.run_script_test()
            #return gb_path_name
        ...
        def build(self):
            ...
            #v.bind(path=lambda *x: pprint(x[1]))
            
            v.bind(path=lambda *x: FileChooserApp.parse_fp(x[1]))
